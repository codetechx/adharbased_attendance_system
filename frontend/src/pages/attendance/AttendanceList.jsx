        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="text-center px-4 py-3 font-medium text-gray-500 hidden sm:table-cell">Method</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} className="text-center py-12 text-gray-400">No records for this date.</td>
              </tr>
            ) : data?.data?.map((log) => (
              <tr key={log.id} className="hover:bg-gray-50/50">
                <td className="px-6 py-3 font-medium text-gray-900">{log.worker?.name}</td>
                <td className="px-4 py-3 text-gray-600 hidden md:table-cell">{log.company?.name}</td>
                <td className="px-4 py-3 text-center">
                  <span className={`badge ${log.type === "IN" ? "badge-green" : "badge-blue"}`}>
                    {log.type === "IN" ? <LogIn size={10} className="mr-1" /> : <LogOut size={10} className="mr-1" />}
                    {log.type}
                  </span>
                </td>
                ...
                <td className="px-4 py-3 text-center hidden sm:table-cell">
                  {log.method === "fingerprint"
                    ? <span className="badge badge-green text-xs">Fingerprint</span>
                    : <span className="badge badge-yellow text-xs flex items-center justify-center gap-1"><Shield size={10} />Manual</span>
                  }
                </td>
              </tr>
            ))}
